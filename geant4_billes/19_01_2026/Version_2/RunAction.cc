...
: G4UserRunAction(),
  ...
  fConeAngle(45.*deg),
  fSourcePosZ(73.5*mm),          // Source à z = 73.5 mm (26.5 mm avant surface eau à z=100 mm)
  fMeanGammasPerDecay(0.7631),   // Am-241: somme des intensités X/gamma (~76.31%)
...
void RunAction::BeginOfRunAction(const G4Run* run)
{
    ...
    // Les anneaux (WaterRing_0 à WaterRing_4) ont une épaisseur de 1 mm
    // (deuxième tranche d'eau uniquement, z = 102-103 mm)
    // ═══════════════════════════════════════════════════════════════
    
    const G4double waterThickness = 1.0 * mm;  // Épaisseur des anneaux (tranche 2)
...
void RunAction::EndOfRunAction(const G4Run* run)
{
    ...
    // ═══════════════════════════════════════════════════════════════
    // TABLEAU DES STATISTIQUES PAR RAIE X/GAMMA
    // ═══════════════════════════════════════════════════════════════
    
    oss << "\n╔═══════════════════════════════════════════════════════════════════════════════════════╗\n";
    oss << "║                        STATISTIQUES PAR RAIE X/GAMMA Am-241                           ║\n";
    oss << "╠════════╦════════════╦═══════════╦═══════════════╦══════════════╦══════════════════════╣\n";
...
void RunAction::FillEventDataNtuple(G4int eventID,
                                     ...
                                     G4int nGammaWater)
{
    ...
    analysisManager->FillNtupleDColumn(0, 2, ringEdep_keV[0]);
    analysisManager->FillNtupleDColumn(0, 3, ringEdep_keV[1]);
    analysisManager->FillNtupleDColumn(0, 4, ringEdep_keV[2]);
    analysisManager->FillNtupleDColumn(0, 5, ringEdep_keV[3]);
    analysisManager->FillNtupleDColumn(0, 6, ringEdep_keV[4]);
    analysisManager->FillNtupleIColumn(0, 7, nGammaEmitted);
    analysisManager->FillNtupleIColumn(0, 8, nGammaWater);
    analysisManager->AddNtupleRow(0);
}

// ═══════════════════════════════════════════════════════════════
// REMPLISSAGE DU NTUPLE GammaData (Ntuple 2)
// ═══════════════════════════════════════════════════════════════

void RunAction::FillGammaDataNtuple(G4int eventID,
                                     G4double energy_keV,
                                     G4int lineID,
                                     G4bool reachedWater,
                                     G4bool absorbed)
{
    ...
    analysisManager->FillNtupleDColumn(2, 1, energy_keV);
    analysisManager->FillNtupleIColumn(2, 2, lineID);
    analysisManager->FillNtupleIColumn(2, 3, reachedWater ? 1 : 0);
    analysisManager->FillNtupleIColumn(2, 4, absorbed ? 1 : 0);
