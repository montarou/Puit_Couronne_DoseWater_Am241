...
// Adapté pour la simulation Am-241
...
void plot_container_planes(const char* filename = "output.root") {
    ...
    std::cout << "\n╔═══════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║           ANALYSE DES PLANS CONTAINER - Am-241                ║" << std::endl;
    std::cout << "╠═══════════════════════════════════════════════════════════════╣" << std::endl;
    ...
    std::cout << "\nNtuple precontainer:  " << nEntriesPre << " événements" << std::endl;
    ...
    TCanvas* c1 = new TCanvas("c1", "Plan PreContainerPlane (avant eau)", 1400, 1000);
    c1->Divide(2, 2);
    ...
    c1->Update();
    c1->SaveAs(baseName + "_precontainer.png");
    
    std::cout << "\n>>> Canvas 1 sauvegardé: " << baseName << "_precontainer.png" << std::endl;
    
    // ═══════════════════════════════════════════════════════════════════════
    // CANVAS 2 : Plan PostContainer - Photons TRANSMIS (sortant de l'eau, +z)
    // ═══════════════════════════════════════════════════════════════════════

    TCanvas* c2 = new TCanvas("c2", "Plan PostContainerPlane - Photons transmis (+z)", 1400, 500);
    c2->Divide(2, 1);
    ...
    c2->Update();
    c2->SaveAs(baseName + "_postcontainer_photons_fwd.png");
    
    std::cout << ">>> Canvas 2 sauvegardé: " << baseName << "_postcontainer_photons_fwd.png" << std::endl;
    
    // ═══════════════════════════════════════════════════════════════════════
    // CANVAS 3 : Plan PostContainer - Photons RÉTRODIFFUSÉS (-z)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    TH1D* h_post_nPhotons_back = new TH1D("h_post_nPhotons_back", 
        "Nombre de photons r#acute{e}trodiffus#acute{e}s;N_{#gamma} backscatter (-z);Nombre d'#acute{e}v#acute{e}nements", 
        10, 0, 10);
    h_post_nPhotons_back->SetLineColor(kMagenta+2);
    h_post_nPhotons_back->SetFillColor(kMagenta-9);
    ...
    TH1D* h_post_sumEPhotons_back = new TH1D("h_post_sumEPhotons_back", 
        ...
        100, 0, 100);  // Adapté: max ~69 keV
    h_post_sumEPhotons_back->SetLineColor(kMagenta+2);
    h_post_sumEPhotons_back->SetFillColor(kMagenta-9);
    ...
    c3->Update();
    c3->SaveAs(baseName + "_postcontainer_photons_back.png");
    
    std::cout << ">>> Canvas 3 sauvegardé: " << baseName << "_postcontainer_photons_back.png" << std::endl;
    
    // ═══════════════════════════════════════════════════════════════════════
    // CANVAS 4 : Comparaison entrant vs transmis (PHOTONS)
    // ═══════════════════════════════════════════════════════════════════════

    TCanvas* c4 = new TCanvas("c4", "Comparaison Photons: Entrant vs Transmis", 1400, 500);
    c4->Divide(2, 1);
    ...
    leg1->AddEntry(h_comp_pre_n, "PreContainer (entrant)", "l");
    leg1->AddEntry(h_comp_post_n, "PostContainer (transmis)", "l");
    ...
    TH1D* h_comp_pre_e = new TH1D("h_comp_pre_e", 
        ...
        100, 0, 250);
    TH1D* h_comp_post_e = new TH1D("h_comp_post_e", "", 100, 0, 250);
    ...
    leg2->AddEntry(h_comp_pre_e, "PreContainer (entrant)", "l");
    leg2->AddEntry(h_comp_post_e, "PostContainer (transmis)", "l");
    leg2->SetTextSize(0.035);
    leg2->Draw();
    
    c4->Update();
    c4->SaveAs(baseName + "_comparison_photons.png");
    
    std::cout << ">>> Canvas 4 sauvegardé: " << baseName << "_comparison_photons.png" << std::endl;
    
    // ═══════════════════════════════════════════════════════════════════════
    // CANVAS 5 : Plan PostContainer - Électrons TRANSMIS (+z)
    // ═══════════════════════════════════════════════════════════════════════

    TCanvas* c5 = new TCanvas("c5", "Plan PostContainerPlane - Electrons transmis (+z)", 1400, 500);
    c5->Divide(2, 1);
    
    c5->cd(1);
    gPad->SetLogy();
    TH1D* h_post_nElectrons_fwd = new TH1D("h_post_nElectrons_fwd", 
        "Nombre d'#acute{e}lectrons transmis;N_{e^{-}} transmis (+z);Nombre d'#acute{e}v#acute{e}nements", 
        10, 0, 10);
    ...
    c5->Update();
    c5->SaveAs(baseName + "_postcontainer_electrons_fwd.png");
    
    std::cout << ">>> Canvas 5 sauvegardé: " << baseName << "_postcontainer_electrons_fwd.png" << std::endl;
    
    // ═══════════════════════════════════════════════════════════════════════
    // CANVAS 6 : Plan PostContainer - Électrons RÉTRODIFFUSÉS (-z)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    TH1D* h_post_nElectrons_back = new TH1D("h_post_nElectrons_back", 
        "Nombre d'#acute{e}lectrons r#acute{e}trodiffus#acute{e}s;N_{e^{-}} backscatter (-z);Nombre d'#acute{e}v#acute{e}nements", 
        10, 0, 10);
    ...
    c6->Update();
    c6->SaveAs(baseName + "_postcontainer_electrons_back.png");
    
    std::cout << ">>> Canvas 6 sauvegardé: " << baseName << "_postcontainer_electrons_back.png" << std::endl;
    
    // ═══════════════════════════════════════════════════════════════════════
    // CANVAS 7 : Résumé - Bilan de flux
    // ═══════════════════════════════════════════════════════════════════════

    TCanvas* c7 = new TCanvas("c7", "Bilan de flux", 1000, 700);
    c7->Divide(2, 2);
    
    // Bilan photons
    c7->cd(1);
    TH1D* h_bilan_photons = new TH1D("h_bilan_photons", 
        "Bilan photons;Cat#acute{e}gorie;Nombre moyen / #acute{e}v#acute{e}nement", 4, 0, 4);
    h_bilan_photons->SetFillColor(kOrange-3);
    h_bilan_photons->SetBarWidth(0.8);
    h_bilan_photons->SetBarOffset(0.1);
    
    h_bilan_photons->GetXaxis()->SetBinLabel(1, "Entrant");
    h_bilan_photons->GetXaxis()->SetBinLabel(2, "Transmis");
    h_bilan_photons->GetXaxis()->SetBinLabel(3, "Backscatter");
    h_bilan_photons->GetXaxis()->SetBinLabel(4, "Absorb#acute{e}s");
    
    Double_t mean_pre = h_pre_nPhotons->GetMean();
    Double_t mean_fwd = h_post_nPhotons_fwd->GetMean();
    Double_t mean_back = h_post_nPhotons_back->GetMean();
    Double_t mean_abs = mean_pre - mean_fwd - mean_back;
    
    h_bilan_photons->SetBinContent(1, mean_pre);
    h_bilan_photons->SetBinContent(2, mean_fwd);
    h_bilan_photons->SetBinContent(3, mean_back);
    h_bilan_photons->SetBinContent(4, mean_abs > 0 ? mean_abs : 0);
    
    h_bilan_photons->Draw("BAR");
    
    // Bilan énergie photons
    c7->cd(2);
    TH1D* h_bilan_energie = new TH1D("h_bilan_energie", 
        "Bilan #acute{e}nergie photons;Cat#acute{e}gorie;#acute{E}nergie moyenne (keV) / #acute{e}v#acute{e}nement", 4, 0, 4);
    h_bilan_energie->SetFillColor(kCyan-3);
    h_bilan_energie->SetBarWidth(0.8);
    h_bilan_energie->SetBarOffset(0.1);
    
    h_bilan_energie->GetXaxis()->SetBinLabel(1, "Entrant");
    h_bilan_energie->GetXaxis()->SetBinLabel(2, "Transmis");
    h_bilan_energie->GetXaxis()->SetBinLabel(3, "Backscatter");
    h_bilan_energie->GetXaxis()->SetBinLabel(4, "D#acute{e}pos#acute{e}e");
    
    Double_t E_pre = h_pre_sumEPhotons->GetMean() * h_pre_sumEPhotons->GetEntries() / nEntriesPre;
    Double_t E_fwd = h_post_sumEPhotons_fwd->GetMean() * h_post_sumEPhotons_fwd->GetEntries() / nEntriesPost;
    Double_t E_back = h_post_sumEPhotons_back->GetMean() * h_post_sumEPhotons_back->GetEntries() / nEntriesPost;
    Double_t E_dep = E_pre - E_fwd - E_back;
    
    h_bilan_energie->SetBinContent(1, E_pre);
    h_bilan_energie->SetBinContent(2, E_fwd);
    h_bilan_energie->SetBinContent(3, E_back);
    h_bilan_energie->SetBinContent(4, E_dep > 0 ? E_dep : 0);
    
    h_bilan_energie->Draw("BAR");
    
    // Efficacités
    c7->cd(3);
    TH1D* h_efficacite = new TH1D("h_efficacite", 
        "Efficacit#acute{e}s;Cat#acute{e}gorie;Pourcentage (%)", 3, 0, 3);
    h_efficacite->SetFillColor(kGreen-3);
    h_efficacite->SetBarWidth(0.8);
    h_efficacite->SetBarOffset(0.1);
    
    h_efficacite->GetXaxis()->SetBinLabel(1, "Transmission");
    h_efficacite->GetXaxis()->SetBinLabel(2, "Backscatter");
    h_efficacite->GetXaxis()->SetBinLabel(3, "Absorption");
    
    Double_t eff_trans = (mean_pre > 0) ? 100.0 * mean_fwd / mean_pre : 0;
    Double_t eff_back = (mean_pre > 0) ? 100.0 * mean_back / mean_pre : 0;
    Double_t eff_abs = (mean_pre > 0) ? 100.0 * mean_abs / mean_pre : 0;
    
    h_efficacite->SetBinContent(1, eff_trans);
    h_efficacite->SetBinContent(2, eff_back);
    h_efficacite->SetBinContent(3, eff_abs > 0 ? eff_abs : 0);
    h_efficacite->SetMaximum(100);
    
    h_efficacite->Draw("BAR");
    ...
    pt->AddText("#bf{BILAN DE FLUX - Am-241}");
    pt->AddText("");
    pt->AddText(Form("#gamma entrant:    %.4f / evt", mean_pre));
    pt->AddText(Form("#gamma transmis:   %.4f / evt (%.1f%%)", mean_fwd, eff_trans));
    pt->AddText(Form("#gamma backscatter: %.4f / evt (%.1f%%)", mean_back, eff_back));
    pt->AddText(Form("#gamma absorb#acute{e}s:   %.4f / evt (%.1f%%)", mean_abs > 0 ? mean_abs : 0, eff_abs > 0 ? eff_abs : 0));
    pt->AddText("");
    pt->AddText(Form("E entrant: %.2f keV/evt", E_pre));
    pt->AddText(Form("E d#acute{e}pos#acute{e}e: %.2f keV/evt", E_dep > 0 ? E_dep : 0));
    
    pt->Draw();
    
    c7->Update();
    c7->SaveAs(baseName + "_bilan_flux.png");
    
    std::cout << ">>> Canvas 7 sauvegardé: " << baseName << "_bilan_flux.png" << std::endl;
    
    // ═══════════════════════════════════════════════════════════════════════
    // Sauvegarde PDF multi-pages
    // ═══════════════════════════════════════════════════════════════════════
    
    TString pdfName = baseName + "_container_planes.pdf";
    c1->Print(pdfName + "(", "pdf");
    c2->Print(pdfName, "pdf");
    c3->Print(pdfName, "pdf");
    c4->Print(pdfName, "pdf");
    c5->Print(pdfName, "pdf");
    c6->Print(pdfName, "pdf");
    c7->Print(pdfName + ")", "pdf");
    
    std::cout << "\n>>> PDF multi-pages sauvegardé: " << pdfName << std::endl;
    ...
    std::cout << "\n╔════════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║                    STATISTIQUES DES FLUX                        ║" << std::endl;
    std::cout << "╠════════════════════════════════════════════════════════════════╣" << std::endl;
    std::cout << "║ PHOTONS:                                                        ║" << std::endl;
    std::cout << std::fixed << std::setprecision(4);
    std::cout << "║   Entrant (pre):      Mean = " << mean_pre << " /evt" << std::endl;
    std::cout << "║   Transmis (post+z):  Mean = " << mean_fwd << " /evt (" 
              << std::setprecision(1) << eff_trans << "%)" << std::endl;
    std::cout << std::setprecision(4);
    std::cout << "║   Backscatter (-z):   Mean = " << mean_back << " /evt (" 
              << std::setprecision(1) << eff_back << "%)" << std::endl;
    std::cout << std::setprecision(4);
    std::cout << "║   Absorbés (calc):    Mean = " << (mean_abs > 0 ? mean_abs : 0) << " /evt (" 
              << std::setprecision(1) << (eff_abs > 0 ? eff_abs : 0) << "%)" << std::endl;
    std::cout << "╠════════════════════════════════════════════════════════════════╣" << std::endl;
    std::cout << "║ ÉNERGIE PHOTONS:                                               ║" << std::endl;
    std::cout << std::setprecision(2);
    std::cout << "║   Entrant:            " << E_pre << " keV/evt" << std::endl;
    std::cout << "║   Transmis:           " << E_fwd << " keV/evt" << std::endl;
    std::cout << "║   Backscatter:        " << E_back << " keV/evt" << std::endl;
    std::cout << "║   Déposée (calc):     " << (E_dep > 0 ? E_dep : 0) << " keV/evt" << std::endl;
    std::cout << "╠════════════════════════════════════════════════════════════════╣" << std::endl;
    std::cout << "║ ÉLECTRONS:                                                      ║" << std::endl;
    std::cout << std::setprecision(4);
    std::cout << "║   Entrant (pre):      Mean = " << h_pre_nElectrons->GetMean() << " /evt" << std::endl;
    std::cout << "║   Transmis (post+z):  Mean = " << h_post_nElectrons_fwd->GetMean() << " /evt" << std::endl;
    std::cout << "║   Backscatter (-z):   Mean = " << h_post_nElectrons_back->GetMean() << " /evt" << std::endl;
    std::cout << "╚════════════════════════════════════════════════════════════════╝" << std::endl;
